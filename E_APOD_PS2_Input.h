...
[Common Controls]
  ...
  - L2 + D-Pad Left,      Decrease gripper torque (added)
  - L2 + D-Pad Right,     Increase gripper torque (added)
  - L1,                   Mandibles/gripper Open (hold to move)
  - L2,                   Mandibles/gripper Close (hold to move)
  - R3,                   Toogle Full/Half head rotation range (NOT WALKMODE)

  - X-Cross,              Toggle: Walk Mode,Translate Mode, GP Player Mode, Single Leg Mode
  - O-Circle,             Toggle Rotate mode
  - Triangle,             Move body to 35 mm from the ground(walk position)
...
word              TorqueLevel;
byte              TorqueIndex;
boolean           MandibleOpen;
boolean           MandibleClose;
...
//extern void      SingleLegMode(void);
extern void      DEBUGPrintMode(void);
//extern boolean   UCheckDistance(void);
//void             DoAttack(void);
//------------------------------------------------------------------------------
...
InputController  g_InputController;        // create Input controller 
 ...
 //-------------------------------------------------------------------------------
  ...
  POTCtrlPos = 50;                         // Default Mandible position
  TorqueIndex = 2;
  //TorqueSelect[2]=128; cMandibleFSRminTorque=300; cTorqueMultiFactor=176
...
//            }  
    ...
    //** [SWITCH MODES]********************************************************
      ...
      //** Toogle Full/Half Head rotation range
      if (ps2x.ButtonPressed(PSB_R3) && ControlMode != WALKMODE) {     // R3 Button Test    #### Vad1
        MSound(1, 50, 2000);                       // [50\4000]
      ...
      if(!fAttackEnable){
        ...
        if (ControlMode == WALKMODE) {
          ...
          if (WalkMethod == 0){                                            //(Walk Method == 0)
            ...
            HeadControl((ps2x.Analog(PSS_RX)-128)/5, HeadPanInput1, (ps2x.Analog(PSS_RY)-128) ); // ### Vad1 (HeadRot, HeadPan, HeadTilt)
        ...
        if (ControlMode == TRANSLATEMODE) {
          ...
          HeadRotAngle1  =  g_InControlState.BodyPos.z*2;
...
// [PS2TurnRobotOff]
//-------------------------------------------------------------------------------
void PS2TurnRobotOff(void){                      // Turn off
...
//-------------------------------------------------------------------------------
// [IndirectDualShock]
// For an indirect control of A-Pod in rotation mode, necessary for obtaining a 
...
//-------------------------------------------------------------------------------
// [Head control], return: HeadRotOut1, HeadPanOut1, HeadTiltOut1
//-------------------------------------------------------------------------------
void HeadControl (short HeadRotInput1, short HeadPanInput1, short HeadTiltInput1) {
  	
  HeadRotOut1  = SmoothControl ((-HeadRotInput1*7/(2-FullHeadRange)), HeadRotOut1, 2);           // '44,8 deg half range and 89,6 deg at full range
  HeadPanOut1  = SmoothControl ((-HeadPanInput1*3), HeadPanOut1, 2);                             // '38,4 deg
