...
*       O-Circle             Start/Stop Record (Hold 5sec)
...
void Control_PS2_Input(void){
	...
	if ((Ps2x.Analog(1) & 0xf0) == 0x70) {
		...
		if (fArmOn) {
			...
			// Record
			if (!Ps2x.Button(PSB_CIRCLE)) {		                              // PSB_CIRCLE - (O) not pressed
			...
			if ((Ps2x.Button(PSB_CIRCLE)) && (mode != 'R' && !fRecStart_10s)) {	              // PSB_CIRCLE - (O) hold pressed 10sec
			...
			else if ((Ps2x.Button(PSB_CIRCLE)) && (mode == 'R' && !fRecStop_10s)) {	          // PSB_CIRCLE - (O) hold pressed 10sec
			...
			if (fRecStart_10s) {
				...
				if (Ps2x.ButtonPressed(PSB_CIRCLE)) {		                  // PSB_CIRCLE - (O) Button Test
