...
float BA_pos = READY_BA;            // Base angle. Servo degrees - 0 is fully CCW
 ...
 float BA_pos_n;                     // Base position new (2D)
...
#define cTorqueMultiFactor     ((cMandibleFSRmaxTorque - cMandibleFSRminTorque) * 100 / 255) // (=176)

// Servo PWM MIN/MAX values
#define cMandClosedPWM	1495
#define cMandMidPWM		1050
#define cMandOpenPWM	890
#define MAND_READY		1370
#define MAND_OFF		1450
...
word	Gr_pos_us = cMandClosedPWM;                     //PWM=1495
word	old_Gr_pos_us;
boolean	MandibleFSR_Activated = false;
byte	POTSavePos;
			...
			if (Ps2x.Button(PSB_L2)) {                   // L2 Button Test
				...
				if (Ps2x.ButtonPressed(PSB_PAD_RIGHT)) {   // (L2 + D-Right), D-Right - Button Test
					...
					if (TorqueIndex < 4) {
						...
						TorqueLevel = (cMandibleFSRminTorque + (TorqueSelect[TorqueIndex] * cTorqueMultiFactor) / 100);
					}
				}  
...
void writeCommand(void) {
	...
	if (mode == 'R') {
		...
		if (myFile.open(name, O_RDWR | O_CREAT | O_AT_END)) {
			...
			SaveOldPos();                       // old_BA_pos_us = BA_pos_us ....
...
void setup(){
	...
	TorqueLevel = (cMandibleFSRminTorque + (TorqueSelect[TorqueIndex] * cTorqueMultiFactor) / 100); //=228
	...
	if(fArmOn == true) {
		...
		if (arm_move) {
			...
			if (doArmIK(0, y_tmp, z_tmp, ga_tmp) == IK_SUCCESS) {
				...
				ServoUpdate(0, BA_pos_us, shl_pos_us, shl1_pos_us, elb_pos_us, wri_pos_us, WRro_pos_us, Gr_pos_us);
...
void DegToUsAll(void) {
...
#ifdef DEBUG
	Serial.println(F("Fn_DegToUsAll"));
	...
	BA_pos_n = BA_pos;  // Vad!
	WRro_pos_n = WRro_pos;
	POTCtrlPos_n = POTCtrlPos;
}



// Writes Servo Position ant group move to position during MoveTime
void ServoUpdate(unsigned int MoveTime, int BA_us, int shl_us, int shl1_us, int elb_us, int wri_us, int WRro_us, int Gr_us) {
...
void servo_park(int park_type) {
	...
	switch (park_type) {
		...
		case PARK_READY:
			...
			Gr_pos_us = MAND_READY;    // POT_START_POS=50, cMandReadyPWM=1370
		...
		case PARK_OFF:
			...
			Gr_park_us = MAND_OFF;                  //MAND_OFF=1450
...
void MandibleControl(void) {
	...
	MandibleFSRInput = analogRead(FSR_ANALOG_PIN);                // Read FSR
	...
	if(MandibleFSRInput > cMandibleContact) {                     // this is true the mandible are touching the object; cMandibleContact=350  
		...
		if(MandibleFSRInput > TorqueLevel+100) {                  // If to much torque:
			Gr_pos_us --;                                           // open mandible a little bit
		}
		else if (MandibleFSRInput < TorqueLevel-100) {            // If to little torque:
			Gr_pos_us ++;                                           // close mandible a little bit
		}  
		if(POTCtrlPos > (POTSavePos + 10)) {                      // open mandibles
			Gr_pos_us = cMandClosedPWM - POTCtrlPos*5/2;            // 1495 - POTCtrlPos*5/2
		}
		...
		Gr_pos_us = min(max(Gr_pos_us, cMandOpenPWM), cMandClosedPWM); 
	}
		...
		Serial.print(F(" FSR: "));
		Serial.print(MandibleFSRInput, DEC);
		Serial.print(F(" TorqueLevel = "));
		Serial.print(TorqueLevel, DEC);
		Serial.print(F(" TDF:"));
		Serial.println(cTorqueMultiFactor, DEC);
	#endif
