...
float GA_pos = READY_GRA;          // Gripper/Wrish angle Servo (degrees), relative to X/Y plane - 0 is horizontal
float Gr_pos = READY_GR;           // Gripper position jaw opening Servo (degrees) - midpoint is halfway open
float Gr_pos_n;                    // Gripper position new

float y_tmp, z_tmp, ga_tmp;	       // temp. variables
float WRro_pos = READY_WRO;        // Wrist Rotate position Servo (degrees) - midpoint is horizontal
...
int old_BA_pos_us, old_shl_pos_us, old_elb_pos_us, old_wri_pos_us, old_Gr_pos_us, old_WRro_pos_us;
	...
	if ((Ps2x.Analog(1) & 0xf0) == 0x70) {
		...
		if(fArmOn) {
			...
			if (abs(rx_trans) > JS_DEADBAND) {
				...
				BA_pos += ((float)rx_trans / JS_SCALE * Speed * (Y_MIN/Y));
			...
			// Playback - Speed control ( mode == SELECT) 
			else if (mode == 'S') {
				if (Ps2x.ButtonPressed(PSB_PAD_UP) || Ps2x.ButtonPressed(PSB_PAD_DOWN)) {   // PSB_PAD_UP OR PSB_PAD_DOWN Test
			...
			else if (Ps2x.Button(PSB_CIRCLE) && !fPSB_CIRCLE_10s) {	          // PSB_CIRCLE hold pressed 10sec
...
void startRecord(void) {
	...
	if (!myFile.open(name, O_RDWR | O_CREAT | O_AT_END)) {
		...
		//myFile.println("time(0),BA_pos_us,shl_pos_us,shl1_pos_us,elb_pos_us,wri_pos_us,WRro_pos_us,Gr_pos_us");
...
// if mode == Record and (old_position - carrent_positon > THRESHOLD_REC) save all servo position to SD 
...
void Control_PS2_Input_S(void){
	...
	if ((Ps2x.Analog(1) & 0xf0) == 0x70) {
		...
		if (Ps2x.ButtonPressed(PSB_CROSS)) {		// X - Cross Button Test
			...
			if (mode == 'P') {
				if (Ps2x.Button(PSB_PAD_UP) || Ps2x.Button(PSB_PAD_DOWN)) {   // PSB_PAD_UP OR PSB_PAD_DOWN Test
