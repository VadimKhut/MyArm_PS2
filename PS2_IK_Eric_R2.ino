		...
		if (fArmOn) {
			...
			// Playback - Speed control ( mode == SELECT) 
				...
				if (Ps2x.ButtonPressed(PSB_PAD_UP) || Ps2x.ButtonPressed(PSB_PAD_DOWN)) {   // PSB_PAD_UP OR PSB_PAD_DOWN Test
					...
					//Increase Delta Time with -50mS   -->
					if(Ps2x.ButtonPressed(PSB_PAD_UP)) {                      // PSB_PAD_UP Test

						Delta_Time_R -= PLAY_SPEED_INCREMENT;
					}
					
					//-Decrease Delta Time +50mS   <--
					if (Ps2x.ButtonPressed(PSB_PAD_DOWN)) {                   // PSB_PAD_DOWN Test
			...
			else if ((Ps2x.Button(PSB_CROSS)) && (mode == 'R') && !fRecStop_10s) {	          // PSB_CROSS - (X) hold pressed 10sec
